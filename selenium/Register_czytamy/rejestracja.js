...
describe('Rejestracja', function () {
    ...
    it('rejestracja', async function () {
        ...
        await driver.findElement(By.id(types.PHONE_INPUT)).sendKeys(faker.random.number({
            min: 100000000,
            max: 999999999
        }), Key.RETURN);
        ...
        await driver.findElement(By.id(types.APARTMENS_NUMBER_INPUT)).sendKeys(faker.random.number({
            min: 1,
            max: 99
        }), Key.RETURN);
